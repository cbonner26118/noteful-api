...
notesRouter
  ...
  .post(jsonParser, (req, res, next) => {
    const { title, content, date_published, folder_id } = req.body;
...
notesRouter
  ...
  .patch(jsonParser, (req, res, next) => {
    ...
    const noteToUpdate = { title, content, date_published };
    ...
    if (numberOfValues === 0)
      return res.status(400).json({
        error: {
          message: `Request body must contain either 'title', 'content', or 'date_published'`,
        },
      });
